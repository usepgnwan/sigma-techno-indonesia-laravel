...
<script>
    ...
    $('.faq-body').on('click','.creatData, .edit-data',function(e){
        e.preventDefault();
        let id = $(this).data('id');
        let url = "{{route('faq.modal',['opt'=> ':id'])}}";
        url = url.replace(":id", id);
        $.MyModal(url, {},{
                title:  (id =='add' ? 'Tambah' : 'Edit') + ' faq',
                closable: false,
                buttons: [{
                    label: 'Close', 
    ...
    $('.faq-body').on('click','.delete-data',function(e){
        let id = $(this).data('id') 
        let url ="{{ route('faq.delete',['opt'=>':id']) }}";
        url = url.replace(":id", id); 
